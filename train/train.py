...
class Train(object):
    def __init__(
        ...
        self.result_file = os.path.join(
            self.output_dir, f"{self.uniprot_id}_results.json"
        )
        self.results = dict()
        self.scoring = {
            "auc_score": "roc_auc",
    ...
    def get_data(self):
        try:
            ...
            self.decoys = pd.read_csv(
                self.decoy_file, nrows=10 * len(self.actives), header=None
            )
    ...
    def train_svc(self):
        ...
        parameters_sv = {
            ...
            "gamma": [0.1, 1, 10, 100, 1000, "auto"],
        }
        gridsearch_sv = GridSearchCV(
            classifier_sv,
            parameters_sv,
            pre_dispatch="n_jobs",
            scoring=self.scoring,
    ...
    def train_models(self):
        ...
        if len(self.actives) < self.sample_threshold:
            print(
                f"ERROR: NOT ENOUGH ({len(self.actives)} < {self.sample_threshold}) SAMPLES FOUND!"
            )
            return
