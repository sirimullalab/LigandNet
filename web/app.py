    ...
    if request.method in ["GET", "POST"]:
        ...
        try:
            if request.args.get("confidence") is not None:
                prediction = l.get_prediction(
                    smiles, "smiles", float(request.args.get("confidence"))
                )
            else:
                prediction = l.get_prediction(
                    smiles, input_type="smiles", confidence_threshold=0.5
                )

            if prediction is None:
                return jsonify({"message": "PREDICTION ERROR"})
