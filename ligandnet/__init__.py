import os
import joblib
from .ddt.utility import FeatureGenerator
import numpy as np
from tqdm import tqdm


class LigandNet(object):

    __models_dir = os.path.join(os.path.dirname(__file__), "models")
    __best_models = [
        "O00141.xgb",
        "O00206.rf",
        "O00255.svc",
        "O00311.rf",
        "O00329.ann",
        "O00398.xgb",
        "O00400.rf",
        "O00408.svc",
        "O00421.xgb",
        "O00444.rf",
        "O00519.ann",
        "O00748.rf",
        "O00767.svc",
        "O14649.xgb",
        "O14672.ann",
        "O14684.xgb",
        "O14746.xgb",
        "O14757.rf",
        "O14842.svc",
        "O14920.ann",
        "O14939.xgb",
        "O14965.xgb",
        "O15054.xgb",
        "O15055.xgb",
        "O15120.rf",
        "O15151.rf",
        "O15164.xgb",
        "O15229.rf",
        "O15264.xgb",
        "O15379.ann",
        "O60218.rf",
        "O15496.ann",
        "O15530.ann",
        "O15554.rf",
        "O43174.svc",
        "O43318.svc",
        "O43451.xgb",
        "O43613.rf",
        "O43614.rf",
        "O43924.xgb",
        "O75164.rf",
        "O60235.rf",
        "O60341.rf",
        "O60427.ann",
        "O60502.svc",
        "O60563.svc",
        "O60674.xgb",
        "O60725.rf",
        "O60741.rf",
        "O60760.rf",
        "O60885.ann",
        "O60911.rf",
        "O75116.svc",
        "O95749.rf",
        "O75173.ann",
        "O75460.xgb",
        "O75582.xgb",
        "O75762.rf",
        "O75874.rf",
        "O75907.ann",
        "O75908.xgb",
        "O76083.svc",
        "O94925.svc",
        "O94953.rf",
        "O95136.xgb",
        "O95263.rf",
        "O95271.xgb",
        "O95665.svc",
        "P00488.svc",
        "O15382.rf",
        "O95822.svc",
        "O95977.xgb",
        "O96013.svc",
        "O96017.rf",
        "P00338.ann",
        "P00374.svc",
        "P08311.ann",
        "P00491.rf",
        "P00492.svc",
        "P00519.rf",
        "P00734.ann",
        "P00740.ann",
        "P00746.rf",
        "P00750.xgb",
        "P00797.ann",
        "P00813.rf",
        "P01042.rf",
        "P01275.rf",
        "P02708.rf",
        "P02753.svc",
        "P02766.xgb",
        "P03951.ann",
        "P03952.rf",
        "P03956.rf",
        "P04049.rf",
        "P04054.xgb",
        "P04062.rf",
        "P04066.rf",
        "P04150.xgb",
        "P04278.xgb",
        "P04629.svc",
        "P04818.rf",
        "P05089.xgb",
        "P05091.svc",
        "P05093.xgb",
        "P05121.ann",
        "P05129.svc",
        "P05164.rf",
        "P05186.xgb",
        "P05362.rf",
        "P05412.svc",
        "P05413.rf",
        "P05771.xgb",
        "P05981.svc",
        "P06213.svc",
        "P06239.rf",
        "P08684.xgb",
        "P06276.ann",
        "P06401.rf",
        "P06493.ann",
        "P06734.xgb",
        "P06737.xgb",
        "P06756.rf",
        "P06870.svc",
        "P06881.xgb",
        "P07099.ann",
        "P07288.ann",
        "P07333.ann",
        "P07339.xgb",
        "P07384.rf",
        "P07477.rf",
        "P07550.ann",
        "P07711.xgb",
        "P07858.ann",
        "P07900.rf",
        "P07947.svc",
        "P07948.svc",
        "P08069.ann",
        "P08172.rf",
        "P08173.xgb",
        "P08183.rf",
        "P08235.rf",
        "P08238.svc",
        "P08246.xgb",
        "P08254.ann",
        "P10635.svc",
        "P08473.ann",
        "P08575.rf",
        "P08581.ann",
        "P08588.ann",
        "P08631.xgb",
        "P11388.xgb",
        "P08709.rf",
        "P08842.svc",
        "P08908.xgb",
        "P08912.ann",
        "P08913.ann",
        "P08922.ann",
        "P09038.svc",
        "P09211.rf",
        "P09237.svc",
        "P09238.svc",
        "P09467.rf",
        "P09871.xgb",
        "P09874.xgb",
        "P09958.xgb",
        "P09960.svc",
        "P15086.rf",
        "P0DMS9.rf",
        "P10145.xgb",
        "P10275.ann",
        "P10276.xgb",
        "P10619.xgb",
        "P15144.svc",
        "P10721.svc",
        "P10826.rf",
        "P10827.svc",
        "P10828.svc",
        "P11086.ann",
        "P11137.rf",
        "P11150.xgb",
        "P11166.rf",
        "P11169.svc",
        "P11217.rf",
        "P11229.xgb",
        "P11309.svc",
        "P11362.rf",
        "P11387.svc",
        "P16109.svc",
        "P11474.rf",
        "P11509.svc",
        "P11511.xgb",
        "P11597.rf",
        "P0C869.rf",
        "P11766.xgb",
        "P11802.ann",
        "P12268.rf",
        "P12821.rf",
        "P12931.svc",
        "P13497.svc",
        "P13501.svc",
        "P13631.svc",
        "P13807.xgb",
        "P13945.ann",
        "P14061.ann",
        "P14091.xgb",
        "P14174.svc",
        "P14222.rf",
        "P14324.xgb",
        "P14410.xgb",
        "P14416.xgb",
        "P14555.svc",
        "P14625.svc",
        "P14780.ann",
        "P14867.rf",
        "P14902.rf",
        "P14920.svc",
        "P17538.xgb",
        "P15090.xgb",
        "P18507.xgb",
        "P15328.rf",
        "P15428.svc",
        "P15538.svc",
        "P15559.xgb",
        "P16050.svc",
        "P16083.rf",
        "P25021.rf",
        "P16444.xgb",
        "P16455.xgb",
        "P16499.xgb",
        "P16581.svc",
        "P17252.svc",
        "P25098.rf",
        "P17655.ann",
        "P17706.xgb",
        "P17752.xgb",
        "P17931.rf",
        "P17936.rf",
        "P18031.ann",
        "P18054.xgb",
        "P18089.rf",
        "P30305.rf",
        "P18825.ann",
        "P19099.rf",
        "P19793.svc",
        "P19971.xgb",
        "P20292.xgb",
        "P20309.rf",
        "P20618.rf",
        "P20839.rf",
        "P21397.ann",
        "P21452.ann",
        "P21453.rf",
        "P21554.ann",
        "P21728.svc",
        "P21730.svc",
        "P21731.rf",
        "P21802.rf",
        "P21917.xgb",
        "P21918.rf",
        "P21964.rf",
        "P21980.ann",
        "P22001.xgb",
        "P22102.xgb",
        "P22413.rf",
        "P22460.svc",
        "P22607.rf",
        "P22748.rf",
        "P22894.svc",
        "P23141.xgb",
        "P23219.xgb",
        "P23280.xgb",
        "P23415.svc",
        "P23443.rf",
        "P23458.ann",
        "P23467.xgb",
        "P23526.rf",
        "P23786.svc",
        "P23946.ann",
        "P24530.xgb",
        "P24723.rf",
        "P24941.ann",
        "P34903.rf",
        "P25024.xgb",
        "P25025.ann",
        "P36639.xgb",
        "P25100.rf",
        "P25101.rf",
        "P25103.ann",
        "P25105.svc",
        "P25116.rf",
        "P25440.rf",
        "P25774.xgb",
        "P25929.ann",
        "P27338.rf",
        "P27361.svc",
        "P27694.xgb",
        "P27707.rf",
        "P28062.svc",
        "P28074.rf",
        "P28161.xgb",
        "P28221.ann",
        "P28222.ann",
        "P28472.xgb",
        "P28482.ann",
        "P28702.xgb",
        "P28715.xgb",
        "P28838.rf",
        "P29371.rf",
        "P29401.rf",
        "P29466.xgb",
        "P29474.ann",
        "P29597.rf",
        "P37088.xgb",
        "P30411.svc",
        "P30419.svc",
        "P30530.svc",
        "P30536.rf",
        "P30559.rf",
        "P30872.rf",
        "P30874.xgb",
        "P30939.ann",
        "P30989.svc",
        "P31391.xgb",
        "P31639.xgb",
        "P31644.xgb",
        "P31749.xgb",
        "P31751.svc",
        "P32238.svc",
        "P32239.xgb",
        "P32245.xgb",
        "P32246.ann",
        "P32247.svc",
        "P33261.svc",
        "P32745.xgb",
        "P32754.xgb",
        "P33032.svc",
        "P51452.xgb",
        "P33316.rf",
        "P33527.svc",
        "P33981.ann",
        "P53609.xgb",
        "P34913.xgb",
        "P34972.ann",
        "P34995.svc",
        "P34998.ann",
        "P35218.ann",
        "P35228.rf",
        "P35346.rf",
        "P35354.xgb",
        "P35367.rf",
        "P35368.rf",
        "P35372.svc",
        "P35408.xgb",
        "P35414.rf",
        "P35557.rf",
        "P35610.rf",
        "P35869.rf",
        "P54760.svc",
        "P36888.svc",
        "P36897.svc",
        "P37023.rf",
        "P37058.xgb",
        "P37059.svc",
        "P55212.svc",
        "P37231.rf",
        "P37268.svc",
        "P39086.ann",
        "P39748.svc",
        "P39900.svc",
        "P41143.rf",
        "P41145.ann",
        "P41146.xgb",
        "P41180.ann",
        "P41229.svc",
        "P41279.svc",
        "P41594.rf",
        "P41597.xgb",
        "P41968.rf",
        "P42261.xgb",
        "P42262.ann",
        "P42330.rf",
        "P42336.ann",
        "P42338.ann",
        "P42345.rf",
        "P42356.xgb",
        "P42574.xgb",
        "P42575.xgb",
        "P42785.ann",
        "P42892.svc",
        "P43004.xgb",
        "P43115.xgb",
        "P43116.xgb",
        "P43119.rf",
        "P43166.ann",
        "P43235.ann",
        "P43405.ann",
        "P43490.xgb",
        "P43681.rf",
        "P45452.xgb",
        "P45983.ann",
        "P45984.ann",
        "P46093.xgb",
        "P46663.ann",
        "P47712.svc",
        "P47869.rf",
        "P47870.rf",
        "P47871.xgb",
        "P47898.ann",
        "P47900.rf",
        "P48039.ann",
        "P48048.ann",
        "P48058.rf",
        "P48147.svc",
        "P80365.svc",
        "P48443.svc",
        "P48449.rf",
        "P48736.ann",
        "P49137.rf",
        "P49146.xgb",
        "P49238.rf",
        "P49281.xgb",
        "P49286.rf",
        "P49327.svc",
        "P49336.rf",
        "P49682.ann",
        "P49683.xgb",
        "P49721.ann",
        "P49759.xgb",
        "P49798.rf",
        "P50052.rf",
        "P50281.rf",
        "P50336.xgb",
        "P50391.rf",
        "P50416.ann",
        "P50579.xgb",
        "P50750.svc",
        "P51449.rf",
        "Q04206.xgb",
        "P51512.svc",
        "P51677.rf",
        "P51681.rf",
        "P51685.svc",
        "P51686.rf",
        "P51787.rf",
        "P51812.xgb",
        "P52333.ann",
        "P52895.xgb",
        "P53350.xgb",
        "P53582.rf",
        "Q07973.rf",
        "P53634.ann",
        "P53667.svc",
        "P53671.rf",
        "P53779.svc",
        "P53985.xgb",
        "Q10469.xgb",
        "P55055.ann",
        "P55072.svc",
        "P55157.xgb",
        "P55201.rf",
        "P55210.rf",
        "Q13443.rf",
        "P55263.svc",
        "P55789.ann",
        "P55899.xgb",
        "P56192.rf",
        "P56373.ann",
        "P56524.ann",
        "P56817.rf",
        "P60842.xgb",
        "P61073.svc",
        "P62937.rf",
        "P62942.svc",
        "P62993.rf",
        "P68400.svc",
        "P78527.ann",
        "P78536.xgb",
        "P78540.rf",
        "Q14397.rf",
        "P98073.rf",
        "P98170.ann",
        "Q00534.rf",
        "Q00535.xgb",
        "Q00796.rf",
        "Q00987.xgb",
        "Q01064.svc",
        "Q01432.xgb",
        "Q01726.rf",
        "Q02083.rf",
        "Q02127.rf",
        "Q02156.rf",
        "Q02742.rf",
        "Q02750.svc",
        "Q02763.rf",
        "Q03181.rf",
        "Q15125.rf",
        "Q04609.rf",
        "Q04759.ann",
        "Q04760.ann",
        "Q04828.xgb",
        "Q04912.rf",
        "Q05397.rf",
        "Q05469.xgb",
        "Q05586.svc",
        "Q05655.xgb",
        "Q05823.xgb",
        "Q06124.svc",
        "Q06187.svc",
        "Q06418.xgb",
        "Q07075.svc",
        "Q07817.svc",
        "Q07820.xgb",
        "Q07869.xgb",
        "Q07912.rf",
        "Q16445.rf",
        "Q08345.svc",
        "Q08828.xgb",
        "Q08881.svc",
        "Q09428.xgb",
        "Q16548.rf",
        "Q12791.rf",
        "Q12852.ann",
        "Q12860.rf",
        "Q12866.rf",
        "Q12884.ann",
        "Q12908.rf",
        "Q13002.xgb",
        "Q13085.ann",
        "Q13093.svc",
        "Q13133.xgb",
        "Q13153.rf",
        "Q13224.rf",
        "Q13255.xgb",
        "Q13258.xgb",
        "Q13393.rf",
        "Q16620.rf",
        "Q13464.xgb",
        "Q13510.rf",
        "Q13526.rf",
        "Q13546.xgb",
        "Q13627.rf",
        "Q13639.rf",
        "Q13822.xgb",
        "Q13882.ann",
        "Q13946.ann",
        "Q14164.svc",
        "Q14289.rf",
        "Q6PCB7.rf",
        "Q14416.rf",
        "Q14654.svc",
        "Q14680.ann",
        "Q14790.svc",
        "Q14832.xgb",
        "Q14833.rf",
        "Q15046.xgb",
        "Q15059.svc",
        "Q15118.xgb",
        "Q15119.xgb",
        "Q8N5Z0.rf",
        "Q15139.xgb",
        "Q15303.svc",
        "Q15661.rf",
        "Q15722.xgb",
        "Q15759.svc",
        "Q15761.xgb",
        "Q15910.rf",
        "Q16288.xgb",
        "Q16478.xgb",
        "Q92523.xgb",
        "Q16539.ann",
        "Q92934.rf",
        "Q16581.rf",
        "Q16602.svc",
        "Q99895.rf",
        "Q16665.svc",
        "Q16678.rf",
        "Q16739.ann",
        "Q16769.ann",
        "Q16832.rf",
        "Q16853.rf",
        "Q3SYC2.svc",
        "Q495T6.xgb",
        "Q59H18.xgb",
        "Q5S007.svc",
        "Q5TC84.xgb",
        "Q6P988.xgb",
        "Q9H773.xgb",
        "Q6V1X1.rf",
        "Q7Z2W7.ann",
        "Q7Z601.rf",
        "Q86TI2.rf",
        "Q86V86.xgb",
        "Q86X55.ann",
        "Q8IV61.svc",
        "Q8IXJ6.ann",
        "Q8N1Q1.ann",
        "Q8N474.xgb",
        "Q9H8M2.svc",
        "Q8NER1.ann",
        "Q8NET8.xgb",
        "Q8TCT1.svc",
        "Q8TDS4.svc",
        "Q8TDU6.rf",
        "Q8TDV5.svc",
        "Q8TEK3.svc",
        "Q8WTV0.rf",
        "Q8WUI4.svc",
        "Q8WWL7.rf",
        "Q9HAZ1.xgb",
        "Q92633.xgb",
        "Q92793.svc",
        "Q92847.ann",
        "Q9HC97.ann",
        "Q969F8.svc",
        "Q969S8.rf",
        "Q96DB2.rf",
        "Q96EB6.rf",
        "Q96GD4.svc",
        "Q96IY4.svc",
        "Q96JM7.xgb",
        "Q96KQ7.xgb",
        "Q96KS0.rf",
        "Q96LA8.rf",
        "Q96PY6.xgb",
        "Q96RI1.rf",
        "Q96RJ0.svc",
        "Q99500.svc",
        "Q9NZJ5.xgb",
        "Q9P1W9.xgb",
        "Q9UBN7.xgb",
        "Q9UBX1.svc",
        "Q9UBY5.xgb",
        "Q9UGL1.ann",
        "Q9UGN5.svc",
        "Q9UHC3.rf",
        "Q9UHL4.ann",
        "Q9UIQ6.svc",
        "Q9UKE5.svc",
        "Q9UKV0.rf",
        "Q9ULX7.ann",
        "Q9UNA0.rf",
        "Q9UNQ0.svc",
        "Q9UQB9.rf",
        "Q9Y210.xgb",
        "Q9Y243.rf",
        "Q9Y251.rf",
        "Q9Y271.rf",
        "Q9Y2D0.rf",
        "Q9Y2R2.xgb",
        "Q9Y2T6.xgb",
        "Q9Y3Q4.xgb",
        "Q9Y5Y4.rf",
        "Q9H244.rf",
        "Q9H3R0.xgb",
        "Q9H5J4.xgb",
        "Q9H7Z7.xgb",
        "Q9NYY3.svc",
        "Q9UHC9.xgb",
        "Q9UHD2.svc",
        "Q9UQL6.rf",
        "Q9Y2I1.ann",
        "Q9Y463.rf",
        "Q9Y5Y6.svc",
        "Q9Y5Z0.svc",
        "Q99538.xgb",
        "Q99558.rf",
        "Q99572.xgb",
        "Q99683.rf",
        "Q99685.rf",
        "Q99705.xgb",
        "Q99808.svc",
        "Q99814.rf",
        "Q99873.xgb",
        "Q9BSW2.rf",
        "Q9BY41.rf",
        "Q9BYF1.xgb",
        "Q9GZN0.svc",
        "Q9GZQ6.xgb",
        "Q9GZT9.svc",
        "Q9H228.rf",
        "Q9H237.xgb",
        "Q9H2K2.xgb",
        "Q9H4B4.rf",
        "Q9H6Z9.rf",
        "Q9HBA0.rf",
        "Q9HBH1.rf",
        "Q9HBH9.svc",
        "Q9HBW0.xgb",
        "Q9HCG7.rf",
        "Q9HCR9.ann",
        "Q9NP59.xgb",
        "Q9NPC2.xgb",
        "Q9NPH5.svc",
        "Q9NR22.rf",
        "Q9NR96.svc",
        "Q9NR97.rf",
        "Q9NS75.xgb",
        "Q9NTG7.xgb",
        "Q9NWZ3.svc",
        "Q9NXG6.svc",
        "Q9NYA1.rf",
        "Q9NYK1.ann",
    ]

    def __init__(self):
        self.models = [self.get_model(path) for path in self.__best_models]

    def get_model(self, model_path):
        model = joblib.load(os.path.join(self.__models_dir, model_path))
        return {
            "uniprot_id": os.path.splitext(model_path)[0],
            "model_name": type(model).__name__,
            "model": model,
            "model_info": ",".join(str(model).replace(" ", "").split("\n")),
        }

    def get_features(self, input, input_type):
        # TODO: Add functionality for reading from a smi file containing a bulk of smiles
        ft = FeatureGenerator()
        if input_type == "smiles":
            ft.load_smiles(input)
        elif input_type == "sdf":
            ft.load_sdf(input)
        else:
            raise NotImplementedError

        cmpd_id, features = ft.extract_tpatf()
        return cmpd_id, features.reshape(-1, 2692)

    # Get predictions
    def predict(self, input, input_type, confidence_threshold=0.5, model_info=False):
        results = {}
        cmpd_id, features = self.get_features(input, input_type)
        cmpd_id = np.array(cmpd_id)
        for model in self.models:
            # np.float32 is not json serializable, take float64
            pred = (
                model["model"].predict_proba(features).astype(np.float64)[:, 1].round(2)
            )
            mask = pred >= confidence_threshold
            for _id, _pred in zip(cmpd_id[mask], pred[mask]):
                # Create a dictionary for compound if not exists
                if _id not in results.keys():
                    results[_id] = {}
                # Update the compound result dictionary
                if model_info:
                    results[_id].update(
                        {
                            model["uniprot_id"]: {
                                "pred": _pred,
                                "model_name": model["model_name"],
                                "model_info": model["model_info"],
                            }
                        }
                    )
                else:
                    results[_id].update({model["uniprot_id"]: _pred})
        return results
