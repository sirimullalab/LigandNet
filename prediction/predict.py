...
if len(compound_list) != features.shape[0]:
    ...
    print("       Number of compounds in the list: {} and feature count: {}".format(len(compound_list), features.shape[0]))
    sys.exit(1)
    ...
    for model_name, model in get_models():
        ...
        if type(model).__name__ == "SVC":
            ...
            if np.any(mask):
                ...
                results[model_name] = [(round(c, 2), str(f)) for c, f in zip(c_list, pred[:, 1][mask])]
    return results 
