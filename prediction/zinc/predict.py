...
# Load the models
def get_models():
    for model_name, model_type in MODELS_INFO[MODELS_INFO["version"] == PY_VERSION][
        ["name", "model_type"]
    ].values:
        model_file = os.path.join(MODELS_DIR, model_name)
        if model_type == "RandomForestClassifier":
            model = joblib.load(model_file + ".rfc")
        elif model_type == "SVC":
            model = joblib.load(model_file + ".svc")
        elif model_type == "MLPClassifier":
            model = joblib.load(model_file + ".mlp")
        elif model_type == "XGBClassifier":
            model = pickle.load(open(model_file + ".xgb", "rb"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sdf", action="store", dest="sdf", required=True, help="SDF file location"
    )
    parser.add_argument(
        "--out", action="store", dest="out", required=True, help="Output directory"
    )
    parse_dict = vars(parser.parse_args())

    if not os.path.isfile(parse_dict["sdf"]):
        raise FileNotFoundError(
            errno.ENOENT, os.strerror(errno.ENOENT), parse_dict["sdf"]
        )
    ...
    for _name, _type, _model in get_models():
        ...
        _pred = (
            ...
            if _type == "SVC"
            else _model.predict_proba(features)[:, 1]
        )
